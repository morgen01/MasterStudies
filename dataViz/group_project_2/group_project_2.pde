...
//    1.)  Select a tab to view the rollover function in the plot.                                            //
//    2.)  You can view multiple datasets on the plot by selecting your preference using the tabs.            //
//    3.)  Use your mouse wheel to zoom in and out on any dataset.                                            //
//    4.)  Scrollbar is available on the lower left corner to scroll horizontally.                            //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int priceInterval = 5;  // Interval on X
int priceIntervalMinor = 5;  // Interval on Y
...
float scrollPercent;  //  Scroll percent
...
void setup() {
  ...
  data = new FloatTable("annualprice.tsv");
  ...
  dataMin = 0;
  dataMax = ceil(data.getTableMax() / priceInterval) * priceInterval;
...
void draw() {
  ...
  textSize(20);
  textAlign(CENTER);
  text("Annual Prices of Goods", 600, 20);
...
// adds a scroll bar to the graph
void DrawScrollBar() {
...
void DrawPriceLabels() {
  ...
  for (float v = dataMin; v <= dataMax; v += priceIntervalMinor) {
    if (v % priceIntervalMinor == 0) {     // If a tick mark
      float y = map(v, dataMin, dataMax, plotY2, plotY1);  
      if (v % priceInterval == 0) {        // If a major tick mark
